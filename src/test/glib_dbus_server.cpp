...
void timeout(G_Timer* timer) {
   G_DbusServer* server = (G_DbusServer*)(timer->get_user_data());

   nlohmann::json input3;
   input3["key"] = 789;
   server->publish_signal("/org/example/ExampleObject", "org.example.ExampleInterface", "Signal3", input3);

   nlohmann::json input4;
   input4["key"] = 012;
   server->publish_signal("/org/example/ExampleObject", "org.example.ExampleInterface", "Signal4", input4);
}
...
int main() {
   ...
   G_DbusServer server("/var/run/dbus/dbus_ipc_toolkit.sock", "org.example.ExampleService");
   server.init();
   ...
   server.add_interface("/org/example/ExampleObject","org.example.ExampleInterface",intf);
