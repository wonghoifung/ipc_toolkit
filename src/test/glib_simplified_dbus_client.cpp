...
void client_ready(void* ud) {
    ...
    input1["key"] = 123;
    client->call_method("org.example.ExampleService", "Hello", input1, output1);
    log_info("Hello res: {}", output1.dump());

    nlohmann::json input2,output2;
    input2["key"] = 456;
    client->call_method("org.example.ExampleService", "World", input2, output2);
    log_info("World res: {}", output2.dump());

    nlohmann::json input3;
    input3["key"] = 789;
    client->publish_signal("Signal1", input3);

    nlohmann::json input4;
    input4["key"] = 012;
    client->publish_signal("Signal2", input4);

    client->subscribe_signal("Signal3", signal3);
    client->subscribe_signal("Signal4", signal4);

}

void timeout(G_Timer* timer) {
    G_Simplified_DbusClient* client = (G_Simplified_DbusClient*)(timer->get_user_data());
...
int main() {
    ...
    G_Simplified_DbusClient client("/var/run/dbus/dbus_ipc_toolkit.sock", "org.example.ExampleClient");
